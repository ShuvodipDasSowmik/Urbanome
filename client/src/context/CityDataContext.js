...
export function CityDataProvider({ children }) {
  ...
  const actions = {
    ...
    fetchTemperatureData: async (city) => {
      try {
        dispatch({ type: ACTION_TYPES.SET_SECTION_LOADING, section: 'temperature', payload: true });
    ...
    fetchPrecipitationData: async (city) => {
      try {
        dispatch({ type: ACTION_TYPES.SET_SECTION_LOADING, section: 'precipitation', payload: true });
    ...
    fetchVegetationData: async (city) => {
      try {
        dispatch({ type: ACTION_TYPES.SET_SECTION_LOADING, section: 'vegetation', payload: true });
    ...
    fetchElevationData: async (city) => {
      try {
        dispatch({ type: ACTION_TYPES.SET_SECTION_LOADING, section: 'elevation', payload: true });
    ...
    fetchAirQualityData: async (city) => {
      try {
        dispatch({ type: ACTION_TYPES.SET_SECTION_LOADING, section: 'airQuality', payload: true });
    ...
    updateRiskAssessment: () => {
      ...
      const floodRisk = calculateFloodRisk(nasaData.precipitation, nasaData.elevation);
      const airQualityRisk = calculateAirQualityRisk(nasaData.airQuality);
      const overallRisk = calculateOverallRisk(heatRisk, floodRisk, airQualityRisk);
...
function calculateOverallRisk(heatRisk, floodRisk, airQualityRisk) {
  ...
  const avgRisk = (riskLevels[heatRisk] + riskLevels[floodRisk] + riskLevels[airQualityRisk]) / 3;
